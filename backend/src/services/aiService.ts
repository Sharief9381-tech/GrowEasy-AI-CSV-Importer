...
3. SKIP row if NO email AND NO mobile
4. mobile_without_country_code: digits only
5. Multiple emails → first to email, rest to crm_note
6. Multiple phones → first to mobile, rest to crm_note
7. created_at: valid ISO date string
8. Combine first+last name into name
9. Map: Phone/Cell/Mobile/WhatsApp→mobile, Email Address→email, Full Name/Prospect→name, Stage/Status→crm_status, Remarks/Notes→crm_note, Owner/Assigned→lead_owner, Entry Date/Created Time→created_at

HEADERS: ${JSON.stringify(headers)}
ROWS (${rows.length} records, 0-indexed): ${JSON.stringify(rows)}

Return ONLY valid JSON, no markdown:
{"results":[{"rowIndex":0,"action":"import","record":{"name":"...","email":"...","mobile_without_country_code":"...","country_code":"...","company":"...","city":"...","state":"...","country":"...","lead_owner":"...","crm_status":"GOOD_LEAD_FOLLOW_UP","crm_note":"...","data_source":"...","created_at":"..."}},{"rowIndex":1,"action":"skip","reason":"No email or mobile"}]}`;
}

async function callMistralWithRetry(prompt: string, retries: number = 0): Promise<string> {
  try {
    const response = await getClient().chat.complete({
      model: "mistral-small-latest",
      messages: [
        { role: "system", content: "You are a CRM data extraction assistant. Always respond with valid JSON only. Never include markdown or explanations." },
    ...
    if (retries < MAX_RETRIES) {
      ...
      return callMistralWithRetry(prompt, retries + 1);
    }
    throw error;
  }
}
...
export async function processCSVWithAI(
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    try {
      const raw = await callMistralWithRetry(buildPrompt(headers, batchRows));
      ...
      console.error(`[AI Error] Batch ${i}-${i + batchRows.length}:`, errMsg.slice(0, 150));
      batchRows.forEach((d, idx) => allSkipped.push({ rowIndex: i + idx, reason: errMsg.slice(0, 100), originalData: d }));
    }
