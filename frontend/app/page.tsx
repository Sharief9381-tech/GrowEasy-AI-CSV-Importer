...
export default function Home() {
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      const data = await processCSV(file, (processed, total, percent) => {
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-sm shadow-2xl overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-3">
              {step === 1 && (<><DocumentArrowUpIcon className="w-5 h-5 text-violet-400" /><span className="font-semibold">Upload CSV</span><span className="text-gray-500 text-sm">— Step 1 of 4</span></>)}
              {step === 2 && (<><div className="w-5 h-5 text-violet-400"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 10h18M3 6h18M3 14h18M3 18h18" /></svg></div><span className="font-semibold">Preview Data</span><span className="text-gray-500 text-sm">— Step 2 of 4</span></>)}
              {step === 3 && (<><SparklesIcon className="w-5 h-5 text-violet-400 animate-pulse" /><span className="font-semibold">AI Processing</span><span className="text-gray-500 text-sm">— Step 3 of 4</span></>)}
              {step === 4 && (<><CheckIcon className="w-5 h-5 text-emerald-400" /><span className="font-semibold">Import Complete</span><span className="text-gray-500 text-sm">— Step 4 of 4</span></>)}
            </div>
            {(step === 2 || step === 4) && (
              <button onClick={handleReset} className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-white transition-colors px-3 py-1.5 rounded-lg hover:bg-white/10">
            ...
            {step === 1 && (
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {["Facebook Lead Ads","Google Ads","Excel Exports","Real Estate CRMs","Sales Reports","Any CSV Format"].map(l => (
                    <span key={l} className="text-xs px-2.5 py-1 rounded-full bg-white/5 text-gray-400 border border-white/10">{l}</span>
                  ))}
                </div>
              </div>
            )}
                ...
                <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-violet-600/10 border border-violet-500/20">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400">
                      ...
                      <span className="mx-1.5">·</span>{preview.totalRows} rows
                      <span className="mx-1.5">·</span>{preview.headers.length} columns
                    </p>
                  </div>
                </div>

                <PreviewTable headers={preview.headers} rows={preview.rows} totalRows={preview.totalRows} previewRows={preview.previewRows} />

                <div className="flex items-center gap-4 pt-2">
                  <button
                    onClick={handleConfirmImport}
                    disabled={loading}
                    className="flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 text-white font-semibold transition-all duration-200 shadow-lg shadow-violet-600/25 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <SparklesIcon className="w-4 h-4" />
                    Confirm & Run AI Import
                  </button>
                  <p className="text-xs text-gray-500">AI will extract CRM fields from your data</p>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div className="py-8 space-y-6">
                ...
                <div className="flex flex-wrap justify-center gap-2 text-xs text-gray-600">
                  {["Analyzing column headers","Mapping fields with AI","Validating records","Extracting CRM data"].map((s) => (
                    <span key={s} className="flex items-center gap-1.5 px-2.5 py-1 bg-white/5 rounded-full">
                      <span className="w-1 h-1 rounded-full bg-violet-500 animate-pulse" />{s}
                    </span>
                  ))}
                </div>
              </div>
